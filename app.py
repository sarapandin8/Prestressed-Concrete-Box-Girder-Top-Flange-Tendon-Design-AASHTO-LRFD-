...
# ── Sync Function: Merge data_editor deltas into the source DataFrame ──
...
with st.sidebar:
    ...
    with st.expander("💾  Save  /  📂  Open Project", expanded=True):
        ...
        st.download_button("💾 Save Project (.json)", data=_json_bytes, file_name=_fname, mime="application/json", use_container_width=True)
        ...
        if uploaded_file is not None:
            try:
                ...
                for k, v in loaded.get("scalars", {}).items():
                    if k in DEFAULT_SCALARS: st.session_state[k] = type(DEFAULT_SCALARS[k])(v)
    ...
    with st.expander("🔩 Prestressing Force", expanded=True):
        ...
        st.slider("fpi / fpu", 0.70, 0.80, key="fpi_ratio")
        st.slider("Immediate loss (%)", 0, 15, key="init_loss_pct")
        st.slider("Pe / Pi ratio", 0.50, 0.95, key="eff_ratio")

    with st.expander("⚖️ Resistance Factors"):
...
st.caption("AASHTO LRFD | 1.0 m strip")

c1, c2 = st.columns(2)
with c1:
    st.subheader("📏 Flange Thickness t(x)")
    df_thk = st.data_editor(st.session_state["thk_src"], num_rows="dynamic", key="ed_thk", on_change=sync_table, args=("thk_src", "ed_thk"))
    st.subheader("🔩 Tendon Profile z(x)")
    df_tdn = st.data_editor(st.session_state["tdn_src"], num_rows="dynamic", key="ed_tdn", on_change=sync_table, args=("tdn_src", "ed_tdn"))
with c2:
    st.subheader("📦 Loads per 1 m strip")
    df_ld  = st.data_editor(st.session_state["ld_src"],  num_rows="dynamic", key="ed_ld", on_change=sync_table, args=("ld_src", "ed_ld"))
...
def run_calc(dft, dfp, dfl):
    N = 500; b = 1.0; width = st.session_state.width
