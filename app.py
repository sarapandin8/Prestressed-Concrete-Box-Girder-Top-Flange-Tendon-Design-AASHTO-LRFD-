...
# ── Init table source keys (separate from editor widget keys)
...
# _tbl_ver: increment → editor key changes → widget reinits from updated src (after load)
if "_tbl_ver" not in st.session_state:
    st.session_state["_tbl_ver"] = 0

# _loaded_hash: prevents rerun loop when using static uploader key
...
# ── 💾 SAVE / 📂 OPEN ────────────────────────────────────────────────────
    ...
    with st.expander("💾  Save  /  📂  Open Project", expanded=True):
        ...
        # ── SAVE ─────────────────────────────────────────────────────
        # _cur_thk/_cur_tdn/_cur_ld store the return value of data_editor
        # (plain DataFrame of current user edits — set right after each editor widget)
        ...
        # ── OPEN ─────────────────────────────────────────────────────
        # Static key → file never disappears before handler runs
        # Hash check → process each unique file only once (no rerun loop)
        ...
        if uploaded_file is not None:
            ...
            if st.session_state["_loaded_hash"] != _fhash:
                try:
                    ...
                    # Clear stale cached editor values
...
# _tbl_ver changes on file load → new widget keys → editors reinit from updated src
...
with c1:
    ...
    # Store return value (guaranteed plain DataFrame) for Save block
